        ...
        <>
            <div className=' relative text-white h-full w-full lg:bg-black/5 bg-black/20 flex flex-col items-center    border border-white/50 rounded-xl  shadow-[0_8px_32px_rgba(0,0,0,0.5)] '>
                ...
                <div className='  flex gap-12 pt-24 pb-24 lg:w-full  flex-col overflow-auto no-scrollbar [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)] w-full  items-center'>
                    {/* 🔥 ERROR */}
                    {TodayData === null && (
                        <p className="text-white text-xl">No Extra classes Found !! </p>
                    )}
    ...
    extraClasses.slice().reverse().map((cls, index) => (
        <div key={index} className='w-full relative uppercase lg:text-2xl p-4 bg-black/30 rounded-2xl border border-white/50'>
            ...
            <div className='flex mt-[-16px] mb-4'>
                <span className='bg-blue-600 border-b rounded-b-2xl p-2'>
                    {cls.remarks}
                </span>
            </div>
            ...
            <div className='w-full mt-4 p-2 lg:p-4 bg-white/5 border border-white/50 rounded-2xl flex flex-col gap-4'>
                <h1 className='bg-green-400/20 border border-green-300/50 rounded-2xl flex justify-around'>
                    <span className='font-bold text-green-400'>Sir</span> : {cls.teacher}
                </h1>

                <h1 className='bg-red-400/20 border border-red-300/50 rounded-2xl flex justify-around'>
                    <span className='font-bold text-red-400'>Time</span> : {cls.time}
                </h1>

                <h1 className='bg-yellow-400/20 border border-yellow-300/50 rounded-2xl flex justify-around'>
...
export default ExtraClassTeacher
